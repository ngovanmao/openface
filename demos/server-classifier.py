        ...
        for r in reps:
            ...
            if multiple:
                print("Predict {} @ x={} with {:.2f} confidence.".format(person.decode('utf-8'), bbx,
                                                                         confidence))
                results.append({'person':person.decode('utf-8'),'bbx':bbx, 'confidence':confidence})
            else:
                print("Predict {} with {:.2f} confidence.".format(person.decode('utf-8'), confidence))
                results.append({'person':person.decode('utf-8'),'confidence': confidence})
...
class MyTCPHandler(SocketServer.BaseRequestHandler):
    ...
    override the handler() method to implement communication to the
    client.
    """
    def handle(self):
        global i
